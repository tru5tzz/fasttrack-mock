...
void handle_dcd_config(sl_btmesh_msg_t *evt) {
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    case sl_btmesh_evt_config_client_appkey_status_id:
      /**
       * After the key successfully deployed to one node,
       * start get the DCD data to find out the number of
       * elements and models it has
       * 
       */
      req_handle = evt->data.evt_config_client_appkey_status.handle;
      sl_btmesh_config_client_get_request_status(req_handle, &server_address, NULL, NULL, NULL, NULL);
      app_log("Retrieving the server address from the request handle: %x\n", 
                                                server_address);
      break;
    default:
      break;
